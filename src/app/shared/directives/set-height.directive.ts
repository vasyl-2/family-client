  ...
  ngAfterViewInit() {
    ...
    this.present.pipe(filter(Boolean)).subscribe((p) => {
      ...
      if (img) {
        ...
        const calculated = (height + this.grid!.rowGap) / (this.grid!.rowHeight + this.grid!.rowGap);
        this.rowSpan = Math.ceil(calculated);
        this.renderer.setStyle(this.el.nativeElement, 'gridRowEnd', `span ${this.rowSpan}`);
